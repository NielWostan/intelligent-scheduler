...
export default async function Schedule() {
  ...
  const week1 = formattedGames[0];
  console.log(week1);

  function printGames(division: string, games: any) {
    return (
      <>
        <p>{division}</p>
        {mensAGames.map((weeks: any, index: number) => {
          return (
            <>
              <p>Week {index + 1}</p>
              {weeks.map((game: any, index: number) => {
                return (
                  <>
                    <p>
                      {game[0].team_name} VS {game[1].team_name}
                    </p>
                  </>
                );
              })}
